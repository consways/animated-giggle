...
// - MONGOURL_ENV (required)
// - BLOGNAME (optional, default: HourlyHouston)
...
export default async function handler(req, res) {
  ...
  // Use MONGOURL_ENV and BLOGNAME from .env
  const uri = process.env.MONGOURL_ENV;
  if (!uri) {
    res.status(500).json({ error: 'MONGOURL_ENV not configured' });
    return;
  }

  const dbName = process.env.BLOGNAME || 'HourlyHouston';
